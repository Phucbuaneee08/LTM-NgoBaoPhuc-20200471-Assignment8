...
void *client_thread(void *param)
{
    ...
    else
    {
        ...
        char *message = "HTTP/1.1 200 OK\r\nContent-Type: text/html\r\nConnection: close\r\n\r\n<html>";
        send(clientSocket, message, strlen(message), 0);
        sprintf(buff, "<h1><a style=\"font-weight:900;color:#02308B\" href=\"http://localhost:%d\">.</a></h1>", port);
        send(clientSocket, buff, strlen(buff), 0);
        char *tmp;
        if ((tmp = getParentPath(cwd)) != NULL)
        {
            sprintf(buff, "<h1><a style=\"font-weight:900;color:#11008B\" href=\"http://localhost:%d/get?dir=%s\">..</a></h1>", port, tmp);
            send(clientSocket, buff, strlen(buff), 0);
        }
        ...
        while ((entry = readdir(dir)) != NULL)
        {
            if (strcmp(entry->d_name, ".") != 0 && strcmp(entry->d_name, "..") != 0)
            {
                ...
                {
                    sprintf(buff, "<h1><i><a style=\"font-weight:100;color:#00d233\" href=\"http://localhost:%d/get?file=%s/%s\">%s</a></i></h1>", port, cwd, entry->d_name, entry->d_name);
                    send(clientSocket, buff, strlen(buff), 0);
                }
                else
                {
